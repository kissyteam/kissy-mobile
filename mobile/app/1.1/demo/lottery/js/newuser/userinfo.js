...
KISSY.add('mobile/newuser/userinfo' , function(S , Layout , Tool) {
	...
	C.NewUser = {
		...
		handleBet: function(obj){
			//更新成功
			if(obj.status){
				location.href = decodeURIComponent(this.betfrom);
			}else{
	...
	//验证手机号码
	result.checkPhoneNumber = function(val) {	
		if (val == "") {
			return "请输入手机号码";
		}
	...
	//验证身份证号	
	result.checkIdCard = function(val) {
		if (val === "") {
			return "请输入身份证号";
		}
	...
	result.submit = function(){
		var sobj = Tool.searchJSON();
		var sid = sobj.sid;
		Tool.getScript(C.Config.postUserInfo() + '?sid=' + sid + '&fullname=' + encodeURIComponent(encodeURIComponent(this.userName)) + '&idCard=' + this.cardId + '&mobile=' + this.phoneNumber + '&callback=C.NewUser.handleBet');
	};

	result.initialize = function(betfrom){
		this.removeWait();
		var self = this;
		C.NewUser.betfrom = betfrom;
